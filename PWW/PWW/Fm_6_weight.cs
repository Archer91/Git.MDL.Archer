...
namespace PWW
{
	public partial class Fm_6_weight : ZForm
	{
		...
		private void Fm_6_weight_Load(object sender, EventArgs e)
		{
			LastAdd = new Zt_Gold_So_Dtl();
		}

		private void Fm_6_weight_Shown(object sender, EventArgs e)
		{
			//SetCurr("");
			Gsod_Job_No.Focus();
		...
		private bool SetCurr(string Gsoh_No1, string jobNo, int lineNo = -1)
		{
			...
			else
			{
				...
				var lDtl6 = (from ll in blList where ll.Gsod_Lineno == lineNo select ll).ToList();
				if (lDtl6.Count == 0) //new
				{
					curr = Zt_Gold_So_Dtl.CreateNew(lDtlS.V0(), lineNo, "6");
				...
				curr.Gsod_Qty_Si = curr.Gsod_Qty_S6;
		...
		{
			bAlert = false;
			curr.Gsod_Qty_Si = curr.Gsod_Qty_S6 = getCurrSum();
			//车瓷 ,损耗为:9.5
			//if (curr.Gsod_Qty_6 > 0 && ZConv.ToFloat(curr.Gsod_Qty_6Per) > 9.5) bAlert = true;
			//if (curr.Gsod_Qty_6 > 0 && ZConv.ToFloat(curr.Gsod_Qty_ZPer) > 13) bAlert = true;
			if (curr.Gsod_Qty_6_BaoSun != "") bAlert = true; //|| curr.Gsod_Qty_Z_BaoSun != "" 不考虑
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show("确认要删除 牙位：" + blList[iRow].Gsod_Reason + ",车瓷后重量:" + blList[iRow].Gsod_Qty + " 的行?", "系统提示",
		...
		private void Gsod_Qty_Validating(object sender, System.ComponentModel.CancelEventArgs e)
		{
			...
			else if (curr != null && ZConv.ToFloat(Gsod_Qty.Text) != 0)//&& ZConv.ToFloat(Gsod_Qty.Text) != curr.Gsod_Qty
			{
				...
				{
					e.Cancel = true;
					zMessage.Show(Gsod_Qty, "错误: 车瓷后重量 大于 派金金重量!");
				}
			}
		}
		float getCurrSum()
