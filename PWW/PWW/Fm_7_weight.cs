...
namespace PWW
{
	public partial class Fm_7_weight : ZForm
	{
		...
		private void Fm_7_weight_Load(object sender, EventArgs e)
		{
			LastAdd = new Zt_Gold_So_Dtl();
		}

		private void Fm_7_weight_Shown(object sender, EventArgs e)
		{
			//SetCurr("");
			Gsod_Job_No.Focus();
		...
		private bool SetCurr(string Gsoh_No1, string jobNo, int lineNo = -1)
		{
			...
			else
			{
				...
				var lDtl7 = (from ll in blList where ll.Gsod_Lineno == lineNo select ll).ToList();
				if (lDtl7.Count == 0) //new
				{
					curr = Zt_Gold_So_Dtl.CreateNew(lDtlS.V0(), lineNo, "7");
					var vA = Gold_So_DtlDAL.GetByJob_No(jobNo);
					if (vA.Count > 0)
					{
						curr.Gsod_Reason = vA[0].Gsod_Reason;
						curr.Gsod_Tooth_Qty = vA[0].Gsod_Tooth_Qty;
					}
					inEdit = false;
					Gsod_Qty.Focus();
				...
				curr.Gsod_Qty_S5 = Zt_Gold_So_DtlDAL.GetSumByKey_Type(Gsoh_No1, jobNo, mat, "5");
				curr.Gsod_Qty_S6 = Zt_Gold_So_DtlDAL.GetSumByKey_Type(Gsoh_No1, jobNo, mat, "6");
				curr.IsPfm = Zt_Gold_So_DtlDAL.GetIsPfm(Gsoh_No1, jobNo, mat);
				curr.Gsod_Qty_S7 = sQty_S7;
				...
				curr.Gsod_Qty_Si = curr.Gsod_Qty_S7;
				foreach (var zt1 in blList)
				{
					...
					zt1.Gsod_Qty_S6 = curr.Gsod_Qty_S6;
					zt1.Gsod_Qty_S7 = curr.Gsod_Qty_S7;
		...
		{
			bAlert = false;
			curr.Gsod_Qty_Si = curr.Gsod_Qty_S7 = getCurrSum();
			//车瓷 ,损耗为:9.5
			if (curr.Gsod_Qty_7_BaoSun != "" || curr.Gsod_Qty_Z_BaoSun != "") bAlert = true;
			//if (curr.Gsod_Qty_7 > 0 && ZConv.ToFloat(curr.Gsod_Qty_ZPer) > 13) bAlert = true;
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show("确认要删除 牙位：" + blList[iRow].Gsod_Reason + ",抛光后重量:" + blList[iRow].Gsod_Qty + " 的行?", "系统提示",
		...
		private void Gsod_Qty_Validating(object sender, System.ComponentModel.CancelEventArgs e)
		{
			...
			else if (curr != null && ZConv.ToFloat(Gsod_Qty.Text) != 0)//&& ZConv.ToFloat(Gsod_Qty.Text) != curr.Gsod_Qty
			{
				...
				{
					zMessage.Show(Gsod_Qty, "错误: 抛光后重量 大于 派金金重量!");
					e.Cancel = true;
				}
			}
		}
