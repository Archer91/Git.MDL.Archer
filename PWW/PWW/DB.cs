...
//                    (CONNECT_DATA=(SERVER=DEDICATED)(SID=mdlmdms)));"; //Prd environment
        ...
        public static bool IsAdminUser(string vLoginName)
        {
            ...
            try
            {
              ...
              UNION SELECT '" + vLoginName + @"' UARO_ROLE FROM DUAL ) aaa where UARO_ROLE='R_PW_ADMIN'");
        ...
        public static string GetModifyLimitedHours(string vLoginName)
        {
            ...
            {
                dsH = GetDSFromSql("select udc_value from ZT00_UDC_UDCODE where udc_sys_code='PWW' and udc_category='VALUE' and udc_key='MODIFYLIMITEDHOURS' and udc_code='ADMIN'");
            }
            else
            {
                dsH = GetDSFromSql("select udc_value from ZT00_UDC_UDCODE where udc_sys_code='PWW' and udc_category='VALUE' and udc_key='MODIFYLIMITEDHOURS' and udc_code='NORMAL'");
            }
        ...
        public static string GetEffDateLimitedDays(string vLoginName)
        {
            ...
            if (IsAdminUser(vLoginName))
            {
                dsH = GetDSFromSql("select udc_value from ZT00_UDC_UDCODE where udc_sys_code='PWW' and udc_category='VALUE' and udc_key='EFFDATELIMITEDDAYS' and udc_code='ADMIN'");
            }
            else
            {
                dsH = GetDSFromSql("select udc_value from ZT00_UDC_UDCODE where udc_sys_code='PWW' and udc_category='VALUE' and udc_key='EFFDATELIMITEDDAYS' and udc_code='NORMAL'");
            }
            result = dsH.Tables[0].Rows[0][0].ToString();
            return result;
        }
