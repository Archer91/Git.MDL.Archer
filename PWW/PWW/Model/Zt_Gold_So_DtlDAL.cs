...
namespace PWW.Model
{
	public partial class Zt_Gold_So_DtlDAL
	{
		...
		public static Zt_Gold_So_Dtl DsToModel(DataSet ds, int i)
		{
			...
			mm.Zgsod_5_Is_Pfm = ZConv.V(ds, "ZGSOD_5_IS_PFM", i);

			return mm;
		}

		public static BindingCollection<Zt_Gold_So_Dtl> GetByKey(string Gsoh_No,string Gsod_Lineno)
		{
			...
			DataSet ds = DB.GetDSFromSql("select rowid,ZT_GOLD_SO_DTL.* from ZT_GOLD_SO_DTL " 
										+ Zt_Gold_So_Dtl.where.Gsoh_No(Gsoh_No).Gsod_Lineno(Gsod_Lineno).ToStr()
		...
		{
			Zt_Gold_So_Dtl vv = Zt_Gold_So_Dtl.N;
			OraDelete oraDelete = new OraDelete("ZT_GOLD_SO_DTL");
		...
		public static BindingCollection<Zt_Gold_So_Dtl> GetByKey0(string Gsod_Job_No,string Gsod_Mat_Code)
		{
			BindingCollection<Zt_Gold_So_Dtl> ll = new BindingCollection<Zt_Gold_So_Dtl>();
			DataSet ds = DB.GetDSFromSql("select rowid,ZT_GOLD_SO_DTL.* from ZT_GOLD_SO_DTL " 
										+ Zt_Gold_So_Dtl.where.Gsod_Job_No(Gsod_Job_No).Gsod_Mat_Code(Gsod_Mat_Code).ToStr()
										+ " and Gsoh_No in (select Gsoh_No from  Gold_So_Hdr where GSOH_STATUS <> 'V') " +
		...
		public static BindingCollection<Zt_Gold_So_Dtl> GetByKey1(string Gsod_Job_No)
		{
			BindingCollection<Zt_Gold_So_Dtl> ll = new BindingCollection<Zt_Gold_So_Dtl>();
			DataSet ds = DB.GetDSFromSql("select rowid,ZT_GOLD_SO_DTL.* from ZT_GOLD_SO_DTL " 
										+ Zt_Gold_So_Dtl.where.Gsod_Job_No(Gsod_Job_No).ToStr()
										+ " and Gsoh_No in (select Gsoh_No from  Gold_So_Hdr where GSOH_STATUS <> 'V') " +
										" order by Gsod_Job_No");
			for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
			{
			    var mm = DsToModel(ds, i);
			    ll.Add(mm);
		...
		public static string DeleteSql1(string Gsod_Job_No)
		{
			Zt_Gold_So_Dtl vv = Zt_Gold_So_Dtl.N;
			OraDelete oraDelete = new OraDelete("ZT_GOLD_SO_DTL");
			oraDelete.AddWhere("Gsod_Job_No", vv.Gsod_Job_No.OraV(Gsod_Job_No));
		...
		public static bool Delete1(string Gsod_Job_No)
		{
			return DB.ExecuteFromSql(DeleteSql1(Gsod_Job_No));
		}
		...
		public static string InsertSql(Zt_Gold_So_Dtl mm)
		{
			OraInsert oraInsert = new OraInsert("ZT_GOLD_SO_DTL");
			...
			oraInsert.AddFields("ZGSOD_5_IS_PFM", mm.Zgsod_5_Is_Pfm.OraV());

			return oraInsert.Sql();
		}
		public static bool Insert(Zt_Gold_So_Dtl mm)
		{
			return DB.ExecuteFromSql(InsertSql(mm));
		}

		public static string EditSql(Zt_Gold_So_Dtl mm)
		{
		...
		public static bool Edit(Zt_Gold_So_Dtl mm)
		{
