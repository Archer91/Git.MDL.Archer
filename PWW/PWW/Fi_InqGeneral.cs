...
namespace PWW
{
    public partial class Fi_InqGeneral : Form
    {
        public Fi_InqGeneral()
        {
            InitializeComponent();
        }
...
where jode_emp_code=emp_code(+)
  ...
  and nvl(jode_status,'1')='1' ");
                ...
                //20140613 remove 
                //strbSql.Append(" order by JOME_WPOS_CODE,JOME_JOBNO,JOME_CRT_ON");
                //20140613 by yf  
                //DataSet dsJome = DB.GetDSFromSql(strbSql.ToString());
                DataSet dsJome = DB.GetDSFromSql("select * from ( " + strbSql.ToString() + " union " + strbSql_deduct.ToString() + " ) a1 order by JOME_WPOS_CODE,JOME_JOBNO,JOME_CRT_ON");
